...
try:
    ...
    if not st.session_state.get("authenticated") and not st.session_state.get("manual_logout"):
        ...
        if found_token:
            ...
            with st.container():
                st.markdown(f"<h1 class='main-header' style='margin-top: 100px; opacity:0.5;'>🏠 家庭管理系统 <span style='font-size: 0.8rem;'>v11.10.2</span></h1>", unsafe_allow_html=True)
    ...
    if not st.session_state["authenticated"]:
        with login_placeholder.container():
            st.markdown(f"<h1 class='main-header' style='margin-top: 50px;'>🏠 家庭管理系统 <span style='font-size: 0.8rem; vertical-align: middle; opacity: 0.5;'>v11.10.2</span></h1>", unsafe_allow_html=True)
    ...
    with c_title:
        st.markdown(f"<h1 class='main-header'>🏠 家庭管理系统 <span style='font-size: 0.8rem; vertical-align: middle; opacity: 0.5;'>v11.10.2</span></h1>", unsafe_allow_html=True)
    ...
    if st.session_state.get("show_recovery_center", False):
        ...
        with tc1:
            st.markdown(f"<h2 style='margin:0; font-size: 1.5rem;'>🛡️ 数据恢复中心 <span style='font-size: 0.8rem; color: #888;'>v11.10.2</span></h2>", unsafe_allow_html=True)
        with tc2:
        ...
        if selected_tab == '📝 家庭事项':
            ...
            # 🛠️ v11.10.2: 计算过去 1 周平均体重或最近一次历史体重
            weight_info = ""
            if not weight_df.empty:
                ...
                if not recent_weights.empty:
                    ...
                    weight_info = f"<span style='font-size: 0.9rem; color: #64748b; font-weight: normal; margin-left: 15px;'>⚖️ 过去1周平均体重：{avg_w:.1f}公斤</span>"
                else:
                    latest_r = weight_df.sort_values(by="record_date", ascending=False).iloc[0]
                    weight_info = f"<span style='font-size: 0.9rem; color: #64748b; font-weight: normal; margin-left: 15px;'>⚖️ 最新的历史体重（超过一周）：{latest_r['weight']:.1f}公斤</span>"
            
            st.markdown(f"<h3 style='margin:0;'>⚖️ 体重记录 {weight_info}</h3>", unsafe_allow_html=True)
            st.markdown("<div style='margin-bottom: 10px;'></div>", unsafe_allow_html=True)
